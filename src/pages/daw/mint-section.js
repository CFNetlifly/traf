                    ...
                    ? (
                        props.wallet.connectedToOperatingNetwork ?
                            <div>
                                ...
                                <button className={`button has-font-audiowide is-cyellow is-rounded has-text-black has-text-weight-bold is-size-4 ${props.txReducer.MINT_DAW_TX.loading ? 'is-loading' : ''} `} type="submit" disabled={!props.wallet.connectedToOperatingNetwork || (webData && webData.balance == 0) || !(webData && webData.isMintingAvailable)} >MINT {formik.values.mintQuantity}</button> &nbsp;
