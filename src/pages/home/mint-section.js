    ...
    const getWebData = async () => {
        if (props.wallet.connectedToOperatingNetwork) {
            ...
            try {
                ...
                webData.giveaway1 = Number(webData.leftNFT);
            } catch (e) {
                ...
                {props.wallet.currentAccount ? (
                    props.wallet.connectedToOperatingNetwork ? (
                        <div>
                            ...
                            <button
                                className={`button has-font-audiowide is-cyellow is-rounded has-text-black has-text-weight-bold is-size-4 ${
                                    props.txReducer.MINT_TX.loading ? 'is-loading' : ''
                                } `}
                                type="submit"
                                disabled={
                                    !props.wallet.connectedToOperatingNetwork || !(webData && webData.mintsLeft > 0)
                                }
                            >
                            ...
                            {webData == null ? null : (
                                <div className="">
                                    <h1 className="subtitle mb-0 has-text-white has-text-weight-bold is-hidden">
                                        Your mints: {webData.userMints}
                                    </h1>
