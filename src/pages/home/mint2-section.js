        ...
        <div>
                    ...
                    props.wallet.connectedToOperatingNetwork ? (
                        <div>
                            <button
                                ...
                                disabled={balancesOfUser == 0 ? true : false}
                            >
                                -
                            </button>{' '}
                            &nbsp;
                            <button
                                className={`button has-font-audiowide is-cyellow is-rounded has-text-black has-text-weight-bold is-size-4 ${
                                    props.mint2Reducer['MINT_EP2_TX'].loading ? 'is-loading' : ''
                                } `}
                                type="submit"
                                disabled={!wallet.connectedToOperatingNetwork || balancesOfUser == 0}
                            >
                            ...
                            {balancesOfUser == 0 ? (
                                <div className="has-text-centered">
                                    <p className="has-text-weight-bold is-size-6 has-text-white has-text-centered">
                                        You don't have any mint pass in your wallet
                                    </p>
                                </div>
                            ) : null}
                        </div>
            ...
            <div className="has-text-centered has-text-weight-bold has-text-light has-font-audiowide is-size-6">
                <div style={{ color: '#d1d1d1' }}>
                    333 tokens available only! <br />
                    <br />
                    Use your mint pass to mint Episode 2 before the public sale
                    <br />
                    Episode 2 token with a pure mint pass = 0.5 ETH
                    <br />
                    Episode 2 token with an impure mint pass = 0.95 ETH
                    <br />
                </div>
                <br />
                ...
                <span className="has-text-warning">
                    {' '}
                    PUBLIC MINT WILL OPEN IN <br /> <CountDown className="is-uppercase" />
                </span>
            </div>
        </div>
    );
